...
namespace UHFDemo
{
    class CCommondMethod
    {
        ...
        public static string ByteArrayToStringNoBlank(byte[] btAryHex, int nIndex, int nLen)
        {
            if (nIndex + nLen > btAryHex.Length)
            {
                nLen = btAryHex.Length - nIndex;
        ...
        public static string MyByteArrayToString(byte[] btAryHex, int nIndex, int nLen)
        {
            if (nIndex + nLen > btAryHex.Length)
            {
                nLen = btAryHex.Length - nIndex;
            ...
            for (int nloop = nIndex; nloop < nIndex + nLen; nloop++)
            {
                if ((nloop == nIndex || nloop == nIndex+1) && btAryHex[nloop] == '#')
                {
                    strTemp = "#";
                    flag |= (byte)(1 << nloop);
                }
                else if (flag == (byte)((1<<3)|(1<<4)) && nloop == nIndex + 2)
                {
                    //strTemp = string.Format("{0}":btAryHex[nloop]);
                    StringBuilder sb = new StringBuilder();
                    char c = (char)btAryHex[nloop];
                    sb.Append(c);
                    strTemp = sb.ToString();
                }
                else
                {
                    strTemp = string.Format("{0:X2}", btAryHex[nloop]);
                }
                strResult += strTemp;
            }
